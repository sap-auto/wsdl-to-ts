...
export function wsdlToObj(WSDLurl: string, dirName = "wsdl") {
  if (config.files.length < 1 && WSDLurl !== "") {
    config.files.push(WSDLurl);
    config.outdir = "./" + dirName;
  }
          ...
          return mkdirpp(dir).then(() => {
            return new Promise((resolve, reject) => {
              ...
              if (config.eslintDisable === null) {
                ...
                fileData.push(
                  "/* eslint-disable " + config.eslintDisable.join(", ") + " */"
                );
              }
